...
# ╔═╡ d059bda1-bb0a-4e9a-844b-e478f36f34b4
begin
	using PlutoUI
	using Plots
	using Images
...
# ╔═╡ 2570bd90-936c-11ed-1d75-db626df1c39f
function step(x)
	if x>0
		return 1.0
	else
...
# ╔═╡ 6541f843-d456-432c-a73f-e7023691a53d
function momentum_of_inertia(img::Matrix{RGB{N0f8}})
	...
	for ix in 1:m[2]
		for iy in 1:m[1]
			x = ix
			y = m[1] - iy + 1
			x_new = (cos(sita + pi/2) * (x-x0) + sin(sita + pi/2) * (y-y0)) * A[iy,ix] / sum(A)
			if x_new > 0
				O_p += x_new^2
			end
			if x_new < 0
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "bd122b0d2f588de7c3f64557221d5e541da9b91c"
